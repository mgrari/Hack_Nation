...
| `project` / `address` / `town` / `zip` / `n_units` / `yr_pis` / `latitude` / `longitude` | HUD LIHTC Database (public), frozen to the Boston-Cambridge metro via `fetch_lihtc_data.py` | Show LIHTC property locations only — no vacancy, rent, or eligibility data exists in this source | Discover page (`GET /properties`), renter-selected town/unit filters only, never ranked; lat/lng plot pins on the Discover map |
